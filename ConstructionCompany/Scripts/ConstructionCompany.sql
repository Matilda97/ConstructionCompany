...
/*drop table Materials, Brigades, Customers, Jobs, Orders
--Создание таблиц
...
DECLARE @Symbol CHAR(52)= 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',
		...
		@Brigadier nvarchar(20), @Composition nvarchar(20), @FullName nvarchar(20), @AddressCustomer nvarchar(20),
...
SET @RowCount=1
	...
	BEGIN
		...
		BEGIN
			...
			SET @Position=RAND()*52
			SET @AddressCustomer=@AddressCustomer + SUBSTRING(@Symbol, @Position, 1)
			SET @Position=RAND()*52
			SET @PassportData=@PassportData + SUBSTRING(@Symbol, @Position, 1)
			SET @i=@i+1
		END

		INSERT INTO dbo.Customers(FullName, AddressCustomer, Telephone, PassportData) 
		SELECT @FullName, @AddressCustomer, RAND()*500 , @PassportData
